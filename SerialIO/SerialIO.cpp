//
// Created by james on 17/02/19.
//

#include "SerialIO.h"
#include "../Message.h"
#include <Arduino.h>

void SerialIO::wait_for_bytes(int num_bytes, unsigned long timeout) {
    unsigned long startTime = millis();
    //Wait for incoming bytes or exit if timeout
    while ((Serial.available() < num_bytes) && (millis() - startTime < timeout)){}
}

void SerialIO::writeMessage(enum Message message) {
    uint8_t *Order = (uint8_t*) &message;
    Serial.write(Order, sizeof(*Order));
}

void SerialIO::write_i8(int8_t num) {
    Serial.write(num);
}

void SerialIO::write_i16(int16_t num) {
    int8_t buffer[2] = {(int8_t) (num & 0xff), (int8_t) (num >> 8)};
    Serial.write((uint8_t*)&buffer, 2*sizeof(int8_t));
}

void SerialIO::write_i32(int32_t num) {
    int8_t buffer[4] = {(int8_t) (num & 0xff), (int8_t) (num >> 8 & 0xff), (int8_t) (num >> 16 & 0xff), (int8_t) (num >> 24 & 0xff)};
    Serial.write((uint8_t*)&buffer, 4*sizeof(int8_t));
}

Message SerialIO::readMessage() {
    return (Message) Serial.read();
}

int8_t SerialIO::read_i8() {
    wait_for_bytes(1, 100); // Wait for 1 byte with a timeout of 100 ms
    return (int8_t) Serial.read();
}

int16_t SerialIO::read_i16() {
    int8_t buffer[2];
    wait_for_bytes(2, 100); // Wait for 2 bytes with a timeout of 100 ms
    read_signed_bytes(buffer, 2);
    return (((int16_t) buffer[0]) & 0xff) | (((int16_t) buffer[1]) << 8 & 0xff00);
}

int32_t SerialIO::read_i32() {
    int8_t buffer[4];
    wait_for_bytes(4, 200); // Wait for 4 bytes with a timeout of 200 ms
    read_signed_bytes(buffer, 4);
    return (((int32_t) buffer[0]) & 0xff) | (((int32_t) buffer[1]) << 8 & 0xff00) | (((int32_t) buffer[2]) << 16 & 0xff0000) | (((int32_t) buffer[3]) << 24 & 0xff000000);
}

void SerialIO::read_signed_bytes(int8_t *buffer, size_t n) {
    size_t i = 0;
    int c;
    while (i < n)
    {
        c = Serial.read();
        if (c < 0) break;
        *buffer++ = (int8_t) c; // buffer[i] = (int8_t)c;
        i++;
    }
}

void SerialIO::logMsg(LogLevel logLevel, String traceMsg) {
    if (_sendTrace) {
        writeMessage(LOG);
        write_i8(static_cast<int8_t >(logLevel));
        traceMsg += "\n";
        Serial.write(traceMsg.c_str());
    }
}

void SerialIO::setTrace(bool sendTrace) {
    _sendTrace = sendTrace;
}